...
Comprehensive backend service for Dash providing:
...
TAP_TO_TRADE_EXECUTOR_ADDRESS=0x...
ONE_TAP_PROFIT_ADDRESS=0x...
VAULT_POOL_ADDRESS=0x...
STABILITY_FUND_ADDRESS=0x...
USDC_TOKEN_ADDRESS=0x...
DASH_STAKING_ADDRESS=0x...
...
pm2 start npm --name "dash-backend" -- start
...
```bash
sudo nano /etc/nginx/sites-available/dash-backend
```
...
```bash
# Create symlink
sudo ln -s /etc/nginx/sites-available/dash-backend /etc/nginx/sites-enabled/
...
# View logs
pm2 logs dash-backend
pm2 logs dash-backend --lines 100

# Restart service
pm2 restart dash-backend

# Stop service
pm2 stop dash-backend

# Monitor resources
pm2 monit

# Update code
cd /var/www/dash-trade/dash-be
sudo git pull
npm install
npm run build
pm2 restart dash-backend
...
pm2 start npm --name "dash-backend" --max-memory-restart 500M -- start
```
